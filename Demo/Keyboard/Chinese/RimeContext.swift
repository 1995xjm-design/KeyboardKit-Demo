...
public extension RimeContext {
  ...
  func start(hasFullAccess: Bool) async {
    ...
    // 首次运行：从 bundle 解压内置方案到沙盒（不依赖 App Group 签名）
    do {
      try FileManager.ensureSandboxRimeDirectories()
    } catch {
      Logger.statistics.error("RIME bootstrap error: \(error.localizedDescription)")
    }

    // 启动（键盘侧始终使用沙盒目录，避免侧载/未签名时 App Group 不可用）
    // 已部署过则做增量检查（fullCheck: false），避免每次呼出键盘都触发全量部署
    let deployed = FileManager.default.fileExists(
      atPath: FileManager.sandboxUserDataDirectory.appendingPathComponent(".kkdeployed").path
    )
    ...
    ), maintenance: true, fullCheck: !deployed)
  ...
  func deploy() throws {
    ...
    if schemas.isEmpty {
      ...
      if !FileManager.default.fileExists(atPath: defaultCustomFilePath) {
        FileManager.default.createFile(atPath: defaultCustomFilePath, contents: nil)
      }

      // 此 API 可获取全部方案列表，不经过 schema_list 参数
      schemas = Rime.shared.getAvailableRimeSchemas().sorted()
      _ = Rime.shared.selectRimeSchemas(schemas.map { $0.schemaId })
    }

    // 提前在部署阶段加载 RimeSwitch hotKey
    ...
    // 将 Sandbox 目录下方案复制到 AppGroup（未配置 App Group 时自动降级到沙盒）
...
public extension RimeContext {
  ...
  /// 确保存在默认输入方案（t9 九宫格优先，其次 rime_ice）
  func ensureDefaultSchema() {
    if currentSchema == nil {
      currentSchema = RimeSchema(schemaId: "t9", schemaName: "中文九键")
    }
    if selectSchemas.isEmpty {
      selectSchemas = [currentSchema!]
    }
  }
