...
final class GatewayConnectionController {
        ...
        forceReconnect: Bool = false) async -> String?
    {
        ...
        guard let target else {
            return String(localized: "Failed to resolve the discovered gateway endpoint.")
        }
        ...
        if tlsRequired, stored == nil {
            ...
            else { return String(localized: "Failed to build TLS URL for trust verification.") }
            ...
            case let .fingerprint(fp):
                ...
                self.pendingTrustPrompt = TrustPrompt(
                    ...
                    isManual: false)
                self.appModel?.gatewayStatusText = String(localized: "Verify gateway TLS fingerprint")
                return nil
        ...
        else { return String(localized: "Failed to build discovered gateway URL.") }
        ...
        guard self.persistActiveGateway(registryEntry) else {
            return String(localized: "Could not save the paired gateway.")
        }
    ...
    func switchToGateway(stableID: String) async -> String? {
        guard let stableID = GatewayStableIdentifier.exact(stableID) else {
            return String(localized: "This paired gateway is no longer available.")
        }
        ...
        case .manual:
            guard let host = entry.host, let port = entry.port else {
                return String(localized: "This paired gateway has an invalid saved endpoint.")
            }
            ...
            guard GatewaySettingsStore.setActiveGateway(stableID: stableID) else {
                return String(localized: "Could not save the active gateway selection.")
            }
            await self.connectManual(
                host: host,
                port: port,
    ...
    private func persistActiveGateway(_ entry: GatewaySettingsStore.GatewayRegistryEntry) -> Bool {
        guard GatewaySettingsStore.upsertGatewayRegistryEntry(entry, activate: true) else {
            self.appModel?.gatewayStatusText = String(localized: "Could not save paired gateway")
            return false
        }
        return true
    }
    ...
    func acceptPendingTrustPrompt() async {
        ...
        guard self.persistTLSFingerprint(prompt.fingerprintSha256, pending.stableID) else {
            self.appModel?.gatewayStatusText = String(localized: "Could not save gateway certificate")
            return
        }
    ...
    func trustRotatedGatewayCertificate(from problem: GatewayConnectionProblem) async -> Bool {
        guard problem.canTrustRotatedCertificate,
            ...
            self.appModel?.gatewayStatusText = String(localized: "Certificate review required")
            return false
        }

        guard self.persistTLSFingerprint(fingerprint, stableID) else {
            self.appModel?.gatewayStatusText = String(localized: "Could not update gateway certificate")
            return false
        }
