        ...
        self.tail = Task { @MainActor in
            ...
            failureLog(
                ...
                finalError ?? NSError(
                    ...
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "Transcript persistence failed")]))
        }
    }
...
final class TalkRealtimeWebRTCSession: NSObject {
        ...
        prefetchedSession: TalkRealtimeClientSession? = nil) async throws
    {
        ...
        guard let returnedVoiceSessionId = session.voiceSessionId else {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 11, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "Gateway did not return a realtime voice session"),
            ])
        }
        ...
        if let requestedVoiceSessionId, requestedVoiceSessionId != returnedVoiceSessionId {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 10, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "Gateway returned a conflicting realtime voice session"),
            ])
        }
        ...
        guard let peer = factory.peerConnection(with: config, constraints: constraints, delegate: self) else {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 2, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "Failed to create WebRTC peer connection"),
            ])
        }
    ...
    private func handleRealtimeEvent(_ event: TalkRealtimeServerEvent) {
        ...
        case "error":
            self.delegate?.realtimeSession(self, didChangeStatus: String(localized: "Realtime error"))
        ...
        do {
            ...
            guard let runId = response.runId ?? response.idempotencyKey else {
                throw NSError(domain: "TalkRealtimeWebRTC", code: 8, userInfo: [
                    NSLocalizedDescriptionKey: String(localized: "Gateway did not return a realtime tool run id"),
                ])
            }
            ...
            let fallbackMessage = confirmationInstruction ?? [
                String(localized: "OpenClaw consult did not finish quickly enough."),
                String(localized: "Give a brief spoken fallback from the realtime conversation"),
    ...
    private func submitControlToolCall(callId: String, argsJSON: String) async {
        ...
        do {
            ...
            let res = try await gateway.request(
                ...
                paramsJSON: json,
                timeoutSeconds: Self.toolCallTimeoutSeconds)
            let message = Self.controlResultMessage(from: res) ?? String(localized: "OpenClaw updated the active run.")
            ...
            self.submitToolResult(callId: callId, result: [
                "error": String(localized: "OpenClaw could not update the active run."),
            ])
        }
        guard !Task.isCancelled, !self.stopped else { return }
        if !self.assistantAudioActive {
    ...
    private static func controlParams(sessionKey: String, argsJSON: String) throws -> [String: Any] {
        ...
        guard let text else {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 20, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "OpenClaw control tool call missing text"),
            ])
        }
        var params: [String: Any] = [
            "sessionKey": sessionKey,
        ...
        return try await withThrowingTaskGroup(of: String.self) { group in
                ...
                for await evt in stream {
                    ...
                    if chatEvent.state == "aborted" {
                        throw NSError(domain: "TalkRealtimeWebRTC", code: 9, userInfo: [
                            NSLocalizedDescriptionKey: String(localized: "OpenClaw realtime tool call aborted"),
                        ])
                    }
                    if chatEvent.state == "error" {
                        throw NSError(domain: "TalkRealtimeWebRTC", code: 10, userInfo: [
                            NSLocalizedDescriptionKey: String(localized: "OpenClaw realtime tool call failed"),
                        ])
                    }
                }
                throw NSError(domain: "TalkRealtimeWebRTC", code: 11, userInfo: [
                    NSLocalizedDescriptionKey: String(localized: "OpenClaw realtime tool event stream ended"),
                ])
            }
            ...
            group.addTask {
                ...
                throw NSError(domain: "TalkRealtimeWebRTC", code: 12, userInfo: [
                    NSLocalizedDescriptionKey: String(localized: "OpenClaw realtime tool call timed out"),
                ])
            }
            guard let result = try await group.next() else {
                throw NSError(domain: "TalkRealtimeWebRTC", code: 13, userInfo: [
                    NSLocalizedDescriptionKey: String(localized: "OpenClaw realtime tool call did not finish"),
                ])
            }
            group.cancelAll()
            return result
        ...
        while Date() < deadline {
            ...
            case "error":
                throw NSError(domain: "TalkRealtimeWebRTC", code: 14, userInfo: [
                    NSLocalizedDescriptionKey: wait.error ?? String(localized: "OpenClaw realtime tool call failed"),
                ])
            case "aborted", "cancelled", "canceled":
                throw NSError(domain: "TalkRealtimeWebRTC", code: 15, userInfo: [
                    NSLocalizedDescriptionKey: wait.stopReason ?? String(localized: "OpenClaw realtime tool call aborted"),
                ])
            case "timeout":
                break
            default:
            ...
            peer.offer(for: constraints) { offer, error in
                if let error {
                    ...
                    continuation.resume(throwing: NSError(domain: "TalkRealtimeWebRTC", code: 3, userInfo: [
                        NSLocalizedDescriptionKey: String(localized: "OpenAI realtime offer creation returned no SDP"),
                    ]))
                }
            }
        }
    ...
    private func exchangeOffer(_ sdp: String, session: TalkRealtimeClientSession) async throws -> String {
        ...
        guard let url = await gateway.resolveGatewayHTTPURL(rawURL) else {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 4, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "Invalid OpenAI realtime offer URL"),
            ])
        }
        ...
        guard let http = response as? HTTPURLResponse else {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 5, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "OpenAI realtime offer returned a non-HTTP response"),
            ])
        }
        let elapsed = Int((ProcessInfo.processInfo.systemUptime - startedAt) * 1000)
        ...
        guard let answer = String(data: data, encoding: .utf8),
            ...
            throw NSError(domain: "TalkRealtimeWebRTC", code: 6, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "OpenAI realtime offer returned an empty SDP answer"),
            ])
        }
        return answer
    }
