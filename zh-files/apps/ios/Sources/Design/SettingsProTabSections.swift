...
private struct AppearanceSettingsScreen: View {
    ...
    var body: some View {
        List {
            ...
            Section {
                ForEach(AppLanguagePreference.allCases) { preference in
                    Button {
                        self.selectLanguage(preference)
                    } label: {
                        Label {
                            HStack {
                                Text(verbatim: preference.displayName)
                                    .font(OpenClawType.body)
                                Spacer()
                                if preference == AppLanguagePreference.saved {
                                    Image(systemName: "checkmark")
                                        .foregroundStyle(OpenClawBrand.accent)
                                }
                            }
                    ...
                    .foregroundStyle(.primary)
                    .accessibilityIdentifier("settings-language-\(preference.rawValue)")
                    .accessibilityValue(
                        preference == AppLanguagePreference.saved ? "Selected" : "")
                }
            } header: {
                Text("Language")
            } footer: {
        ...
        .alert("Restart required", isPresented: self.$showLanguageRestartNotice) {
    ...
    private func selectLanguage(_ preference: AppLanguagePreference) {
        guard preference != AppLanguagePreference.saved else { return }
        AppLanguagePreference.saved = preference
        AppLanguagePreference.applySavedPreference()
        self.showLanguageRestartNotice = true
    }
        ...
        Group {
            ...
            if self.talkProviderSelectionRaw == TalkModeProviderSelection.doubao.rawValue {
                Section("Doubao Credentials") {
                    SecureField("API Key (Ark)", text: self.doubaoAPIKeyBinding)
                        .font(OpenClawType.body)
                        .textInputAutocapitalization(.never)
                        .autocorrectionDisabled()
