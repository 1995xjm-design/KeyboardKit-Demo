...
final class ScreenRecordService: @unchecked Sendable {
        ...
        pts: CMTime)
    {
        guard let imageBuffer = CMSampleBufferGetImageBuffer(sample) else {
            state.withLock { state in
                if state.handlerError == nil {
                    state.handlerError = ScreenRecordError.captureFailed(String(localized: "Missing image buffer"))
                }
            }
            return
        }
        ...
        do {
            ...
            guard writer.canAdd(vInput) else {
                throw ScreenRecordError.writeFailed(String(localized: "Cannot add video input"))
            }
            writer.add(vInput)
            ...
            guard writer.startWriting() else {
                throw ScreenRecordError.writeFailed(
                    writer.error?.localizedDescription ?? String(localized: "Failed to start writer"))
            }
                ...
                self.recordQueue.async {
                    do {
                        ...
                        guard let writer, let videoInput, sawVideo else {
                            throw ScreenRecordError.captureFailed(String(localized: "No frames captured"))
                        }
