...
final class NodeAppModel {
    ...
    var lastShareEventText: String = String(localized: "No share events yet.")
        ...
        Task { [weak self] in
            ...
            await MainActor.run {
                ...
                if self.isBackgrounded {
                    self.gatewayStatusText = String(localized: "Background idle")
    ...
    func setTalkEnabled(_ enabled: Bool) {
        ...
        if enabled, self.auxiliaryAudioCapture != nil {
            UserDefaults.standard.set(false, forKey: "talk.enabled")
            self.talkMode.setEnabled(false)
            self.talkMode.statusText = String(localized: "Finish the active audio capture first")
            return
        }
    ...
    private func requestTalkPermissionUpgrade() {
        ...
        self.talkMode.statusText = String(localized: "Requesting Talk approval")
    ...
    func sendDirectWatchSetup() async throws -> WatchNotificationSendResult {
        ...
        guard self.isOperatorGatewayConnected else {
            throw NSError(domain: "WatchDirectSetup", code: 1, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "Connect the iPhone to a Gateway first."),
            ])
        }
        guard self.hasOperatorAdminScope else {
            throw NSError(domain: "WatchDirectSetup", code: 2, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "The iPhone connection needs operator.admin access."),
            ])
        }
        let status = await watchMessagingService.status()
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: String(localized: "OpenClaw Share Self-Test"),
        ...
        syncSnapshots: Bool = true) async -> Bool
    {
        ...
        if selectedPromptWasResolving, !selectedPromptWriteIsInFlight {
            self.pendingExecApprovalPromptErrorText =
                String(localized: "The previous decision was not recorded. Review and try again.")
        }
        return true
    }
                ...
                if markVisiblePromptStale {
                    self.markPendingExecApprovalTerminal(
                        ...
                        outcome: ExecApprovalOutcome(
                            text: String(localized: "This approval is no longer available."),
                            tone: .warning))
                }
                await self.publishWatchExecApprovalExpired(
                    approvalId: approvalID,
        ...
        alreadyResolved: Bool) -> String
    {
        ...
        case .expired:
            return String(localized: "Approval expired before this decision was applied.")
        case .cancelled:
            return String(localized: "Approval was cancelled before this decision was applied.")
        case .resolvedUnknown:
            return String(localized: "Approval was resolved elsewhere.")
        }
    }
        ...
        resolutionAttempt: ExecApprovalResolutionAttempt? = nil) async -> ExecApprovalResolutionOutcome
    {
        guard let approvalID = Self.validatedApprovalID(approvalId) else {
            return .failed(message: String(localized: "Invalid approval request."))
        }
        ...
        guard GatewayStableIdentifier.matches(
            ...
            return .failed(message: String(localized: "This approval belongs to a different gateway."))
        }
        ...
        guard let context = await self.operatorRouteForExecApproval(sourceReason: resolutionReason),
            ...
            return .failed(message: String(localized: "OpenClaw couldn't connect to the gateway operator session."))
        }
        ...
        if rpcFamily == .legacy {
            guard approvalKind == .exec else {
                return .failed(message: String(localized: "This gateway does not advertise a complete approval API."))
            }
        ...
        do {
            ...
            guard await self.isCurrentGatewaySessionRoute(
                ...
                return .uncertain(
                    message: String(localized: "Decision status is unknown after the gateway operator route changed."))
            }
            if let resolutionAttempt {
                self.markExecApprovalResolutionWriteSettled(resolutionAttempt)
            }
        ...
        operatorRoute: GatewayNodeSessionRoute?) async -> ExecApprovalResolutionOutcome
    {
        ...
        case let .terminal(terminal):
            guard terminal.kind == approvalKind else {
                return .failed(message: String(localized: "The approval kind changed during resolution."))
            }
            return await self.applyCanonicalExecApprovalTerminal(
                terminal,
                appliedHere: false,
...
extension NodeAppModel {
    ...
    func declinePendingAgentDeepLinkPrompt() {
        ...
        self.pendingAgentDeepLinkPrompt = nil
        self.screen.errorText = String(localized: "Deep link cancelled.")
