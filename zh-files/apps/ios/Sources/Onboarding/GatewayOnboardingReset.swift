...
enum GatewayOnboardingReset {
        ...
        defaults: UserDefaults) async -> Bool
    {
        guard await appModel.purgeChatTranscriptCache(gatewayID: gatewayStableID) else {
            appModel.gatewayStatusText = String(localized: "Could not remove offline data for this gateway")
            return false
        }
        ...
        defaults: UserDefaults) -> Bool
    {
        guard let deviceId = DeviceIdentityStore.loadOrCreatePersisted()?.deviceId else {
            appModel.gatewayStatusText = String(localized: "Could not access device identity")
            return false
        }
        ...
        if gatewayStableID != nil {
            guard let resolvedShareDeviceId = DeviceIdentityStore
                ...
                appModel.gatewayStatusText = String(localized: "Could not access share device identity")
                return false
            }
