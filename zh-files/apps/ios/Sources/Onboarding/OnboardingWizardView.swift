...
struct OnboardingWizardView: View {
    ...
    private var issueFallbackMessage: String {
        ...
        case let .pairingRequired(requestId):
            requestId.map { "Pairing required (request \($0))." } ?? String(localized: "Pairing required.")
        case .network:
        ...
        .onChange(of: self.selectedPhoto) { _, newValue in
            ...
            Task {
                guard let data = try? await item.loadTransferable(type: Data.self) else {
                    guard self.scannerResultHandoff.isActive(scanID: scanID) else { return }
                    self.showQRScanner = false
                    self.scannerError = String(localized: "Could not load the selected image.")
                    return
                }
                guard self.scannerResultHandoff.isActive(scanID: scanID) else { return }
                ...
                self.showQRScanner = false
                self.scannerError = String(localized: "No valid QR code found in the selected image.")
            }
        }
    }
...
extension OnboardingWizardView {
    ...
    private func connectStagedGatewaySetupLink() async {
        ...
        guard link.isValidEndpoint else {
            let message = String(localized: "Setup link has an invalid gateway endpoint.")
    ...
    private func clearStagedGatewaySetupLink() {
        ...
        let message = String(localized: "Setup link cleared.")
        self.localConnectionFailure = nil
        self.setupCodeStatus = message
        self.statusLine = message
    }
        ...
        disconnectExistingGatewayForBootstrap: Bool = true) async
    {
        ...
        if setupAuth.hasBootstrapToken {
            guard await GatewayOnboardingReset.prepareForBootstrapPairing(
                ...
                let message = String(localized: "Could not safely replace the gateway's offline data. Try again.")
    ...
    private func retryLastAttempt(silent: Bool = false) async {
        ...
        case .manual, .none:
            ...
            if !silent {
                self.setConnectionFailure(String(localized: "No connection to retry. Check the gateway host and port."))
            }
        }
    }
    ...
    private func gatewayProblemPrimaryActionTitle(_ problem: GatewayConnectionProblem) -> String? {
        GatewayProblemPrimaryAction.title(
            for: problem,
            retryTitle: String(localized: "Retry connection"),
            resetTitle: String(localized: "Scan QR again"))
    }
    ...
    private func handleGatewayProblemPrimaryAction(_ problem: GatewayConnectionProblem) async {
        if problem.suggestsOnboardingReset {
            ...
            self.pairingRequestId = nil
            self.navigate(to: .connect)
            self.openQRScannerFromOnboarding(status: String(localized: "Scan a fresh setup QR code from this gateway."))
            return
        }
